...
class LogAnalyzerAgentWrapper:
    ...
    async def analyze(self, provider: str, model: str, container_name: str, log_level_choice: str, api_key: str = None, base_url: str = None) -> str:
        ...
        async with Client(server_params.url) as mcp_client:
            ...
            try:
                task = (await mcp_client.get_prompt("resume_analyze_logs", {"container_name": container_name,
                                                                            "level": log_level_choice})).messages[
                    0].content.text
            except Exception as e:
